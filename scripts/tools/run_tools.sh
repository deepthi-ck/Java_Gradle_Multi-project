...
if command -v diff-cover >/dev/null 2>&1; then
  find . -path "*/build/reports/jacoco/test/jacocoTestReport.xml" -print0 | xargs -0 -I{} diff-cover {} --compare-branch origin/Java-8 || true
else
...
echo "Tool pipeline finished (Java 8 Gradle multi-project)."
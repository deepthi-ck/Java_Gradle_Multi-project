(function () {
  ...
  async function loadHealthBadge() {
    ...
    try {
      ...
      badge.textContent = "Order project " + (h.status || "UP");
    } catch (e) {
      badge.textContent = "Order project unreachable";
    }
  }
  ...
  async function loadDashboard() {
    ...
    probes.innerHTML =
      serviceRow("product-service", "GET /products (proxied)", productUp) +
  ...
  async function loadTopology() {
    ...
    if (routes) {
      ...
      const orderUp = await probe("/orders");
      routes.innerHTML =
        serviceRow("Product facade", "order-service → product-service · /products", productUp) +
        serviceRow("Order API", "order-service · /orders", orderUp);
    }
  }
  ...
  async function loadOrders() {
    ...
    try {
      ...
      if (!rows.length) {
        tbody.innerHTML = '<tr><td colspan="4" class="empty">No orders yet. Publish a product, then place an order with unit price.</td></tr>';
        return;
      }
      ...
      try {
        const body = {
          lines: [{
            ...
            unitPrice: Number($("#order-unit-price").value)
          }]
        };
